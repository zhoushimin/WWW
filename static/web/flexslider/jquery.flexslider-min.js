      ...
      if (!slider.animating && (slider.canAdvance(target, fromNav) || override) && slider.is(":visible")) {
        ...
        if (!fade) {
          ...
          if (!touch) {
            ...
            slider.slides.eq(slider.currentSlide).css({"zIndex": 1, "display": "none"}).animate({"opacity": 0},800, slider.vars.easing);
            slider.slides.eq(target).css({"zIndex": 2, "display": "block"}).animate({"opacity": 1},800, slider.vars.easing, slider.wrapup);
            // 按钮单击添加数据位置
              $('.banner .dq').html($('.flexslider .slides  li').eq(target).attr('data-d'));
